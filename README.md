...
- **[Bun](https://bun.sh)** runtime is required — the channel bridge (`server.ts`) runs on Bun. Install:
  ```bash
  # macOS / Linux
...
> **Troubleshooting:** If the fakechat plugin shows `Status: ✘ failed` with `Command: bun`, it means Bun is not installed or not in PATH. Install Bun and restart Claude Code.
